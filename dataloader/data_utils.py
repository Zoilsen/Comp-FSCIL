...
def set_up_datasets(args):
    ...
    if args.dataset == 'tier_imagenet':
        import dataloader.tierimagenet.tierimagenet as Dataset
        args.base_class = 351
        args.num_classes= 601
        args.way = 25
        args.shot = 5
        args.sessions = 11

    if args.dataset == 'miniBase_tierNovel':
        import dataloader.miniBase_tierNovel.miniBase_tierNovel as Dataset
        args.base_class = 60
        args.num_classes= 632
        args.way = 26
        args.shot = 5
        args.sessions = 23
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'tier_imagenet':
        trainset = args.Dataset.TierImageNet(root=args.dataroot, train=True,
                                             index=class_index, base_sess=True)
        testset = args.Dataset.TierImageNet(root=args.dataroot, train=False, index=class_index)

    if args.dataset == 'miniBase_tierNovel':
        trainset = args.Dataset.miniBase_tierNovel(root=args.dataroot, train=True,
                                             index=class_index, base_sess=True)
        testset = args.Dataset.miniBase_tierNovel(root=args.dataroot, train=False, index=class_index)
...
def get_base_dataloader_meta(args):
    ...
    if args.dataset == 'tier_imagenet':
        trainset = args.Dataset.TierImageNet(root=args.dataroot, train=True,
                                             index_path=txt_path)
        testset = args.Dataset.TierImageNet(root=args.dataroot, train=False,
                                            index=class_index)

    if args.dataset == 'miniBase_tierNovel':
        trainset = args.Dataset.miniBase_tierNovel(root=args.dataroot, train=True,
                                             index_path=txt_path)
        testset = args.Dataset.miniBase_tierNovel(root=args.dataroot, train=False,
                                            index=class_index)
