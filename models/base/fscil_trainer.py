...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        elif self.args.pretrain_encoder_dir is not None:
            log(self.out_log, 'Loading encoder parameters from: %s'%self.args.pretrain_encoder_dir)
            self.best_model_dict = deepcopy(self.model.state_dict())
            self.encoder_dict = torch.load(self.args.pretrain_encoder_dir)['params']
            num_loaded_params = 0
            for name in self.best_model_dict.keys():
                if 'encoder' in name:
                    self.best_model_dict[name] = self.encoder_dict[name]
                    num_loaded_params += 1
            log(self.out_log, '%s encoder params loaded'%num_loaded_params)
    ...
    def get_optimizer_base(self):
        if self.args.dataset == 'cub200' or 'imageNetPretrain' in self.args.tag:
            ...
            if self.args.pretrain_encoder_dir is not None:
                encoder_params = []; fc_params = []
                for name, param in self.model.named_parameters():
                    if 'encoder' in name:
                        encoder_params.append(param)
                ...
                scaleRatio = 0.01
                optimizer = torch.optim.SGD([{'params': encoder_params, 'lr': self.args.lr_base * scaleRatio},
                                             {'params': fc_params}], 
                                            self.args.lr_base, momentum=0.9, nesterov=True,
                                            weight_decay=self.args.decay)
                log(self.out_log, 'num encoder params = %d, num fc params = %d'%(len(encoder_params), len(fc_params)))
                log(self.out_log, 'scale lr of encoder to %s*lr_base'%(str(scaleRatio)))
    ...
    def train(self):
        ...
        for epoch in range(args.epochs_base):
            ...
            if args.in_domain_feat_cls_weight == 0.0:
                if args.map_metric_option != 'none':
                    # to reuse in_domain classification code for map_metric classification
                    args.in_domain_feat_cls_weight = 9999.0
                    tsl, tsa, ind_va, cmb_va = test(self.model, base_testloader, epoch, args, 0, is_base=True)
                    args.in_domain_feat_cls_weight = 0.0
            ...
            if (epoch+1) % 1 == 0:
                ...
                if args.map_metric_option != 'none':
                    key_acc = vas[-2]
                    if key_acc > feat_max_acc:
                        feat_max_acc = key_acc
                        self.best_model_dict = deepcopy(self.model.state_dict())
                        save_model_dir = os.path.join(args.save_path, 'feat_max_acc.pth')
                        torch.save(dict(params=self.model.state_dict()), save_model_dir)
                    
                    key_acc = ind_vas[-2]
                    if key_acc > map_max_acc:
                        map_max_acc = key_acc
                        self.best_model_dict = deepcopy(self.model.state_dict())
                        save_model_dir = os.path.join(args.save_path, 'map_max_acc.pth')
                        torch.save(dict(params=self.model.state_dict()), save_model_dir)
                ...
                if args.map_metric_option == 'none':
                    ...
                    log_str += ', max acc: {:.4f}| ind: {:.4f}| cmb: {:.4f}'.format(feat_max_acc, map_max_acc, max_acc)
